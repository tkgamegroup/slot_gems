...
func appear():
	...
	tween.tween_property(panel, "scale", Vector2(1.0, 1.0), 0.3 * App.speed).set_ease(Tween.EASE_OUT).set_trans(Tween.TRANS_QUART)
...
func calculate_proc():
	...
	var result = {}
	result["combo_mult"] = App.mult_from_combos(App.combos)
	tween.tween_callback(func():
		SSound.se_calc1.pitch_scale = 1.0 / App.speed
	...
	tween.tween_callback(func():
		...
		calculated_text.position = Vector2((App.resolution.x - calculated_text.size.x) * 0.5, 220)
	)
	SAnimation.jump(tween, calculated_text, 8, 0.5 * App.speed, Callable(), true, false)
	tween.tween_property(panel, "scale", Vector2(0.0, 0.0), 0.3 * App.speed).set_ease(Tween.EASE_OUT).set_trans(Tween.TRANS_QUART)
	tween.tween_property(calculated_text, "global_position", App.status_bar_ui.score_text.get_global_rect().get_center() + Vector2(-calculated_text.size.x * 0.5, 50), 0.5 * App.speed).set_ease(Tween.EASE_OUT).set_trans(Tween.TRANS_QUART)
	
	var score0 = App.score
	tween.tween_callback(func():
		SSound.se_score_counting.pitch_scale = 1.0 / App.speed
		SSound.se_score_counting.play()
	)
	tween.tween_method(func(v):
		App.score = score0 + int(v * result["value"])
		calculated_text.text = "%d" % int((1.0 - v) * result["value"])
	, 0.0, 1.0, 0.5 * App.speed)
	tween.tween_callback(func():
		disappear()
		finished.emit()
	)
