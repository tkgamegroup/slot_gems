...
func _ready() -> void:
	...
	sfx_volume_slider.value = db_to_linear(AudioServer.get_bus_volume_db(Sounds.sfx_bus_index))
	music_volume_slider.value_changed.connect(func(v):
		AudioServer.set_bus_volume_db(Sounds.music_bus_index, linear_to_db(v))
	)
	music_volume_slider.value = db_to_linear(AudioServer.get_bus_volume_db(Sounds.music_bus_index))
